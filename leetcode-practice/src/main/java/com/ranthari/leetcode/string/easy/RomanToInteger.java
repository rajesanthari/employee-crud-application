...
/**
 * 
 * https://leetcode.com/problems/roman-to-integer/ Roman numerals are
...
public class RomanToInteger {
	...
	public static int romanToInt(String s) {
		...
		for (int i = 0; i < length; i++) {
			int integer = getNumber(s.charAt(i));
			if (integer < getNumber(s.charAt(i + 1))) {
				sum -= integer;
			} else {
				sum += integer;
			}
		}
		sum += getNumber(s.charAt(length));
		return sum;
	}
