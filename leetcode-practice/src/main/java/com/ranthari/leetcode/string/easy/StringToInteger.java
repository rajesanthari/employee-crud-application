...
public class StringToInteger {
	public static void main(String[] args) {
		int myAtoi = myAtoi("42");
		System.out.println(myAtoi);
	}

	public static int myAtoi(String s) {

		int firstIndex = getNonSpaceIndex(s);
		if (firstIndex == -1) {
			return 0;
		}
		...
		for (int i = firstIndex; i < length; i++) {
			...
			if (sum * negitivenum > Integer.MAX_VALUE) {
				sum = Integer.MAX_VALUE;
				break;
			} else if (sum * negitivenum < Integer.MIN_VALUE) {
	...
	private static int getNonSpaceIndex(String s) {
		int length = s.length();
